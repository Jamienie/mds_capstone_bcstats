...
from gensim.summarization.summarizer import summarize as gensim_summarize # Version 3.7.1
...
def generate_corpus_from_comments(data, depth=None, name=None, agreement=None, sentences=True):
    ...
    present. It also cleans up the grammar in the text so it more evenly splits into
    sentences. 
    ...
    Returns
    -------
    Returns the combined comments as either a list or string depending on the sentences parameters.
    
    sentences: list
        Returns all the combined comments as a list, where each comment is an item in the list. To
        have the function return this format select True for the sentences parameter.
    text: str
        Returns all the combined comments as a sinlge string. To have it returned in this form
        have the sentences parameter False.
...
def generate_text_summary(input_file, summary_size, depth, name, agreement, 
    ...
    """
    Input the data file and this function will clean, process and summarize the comments. 
    This funcation can summarize all the text or it split to subtheme/theme and agreement
    level. It will also generate an output file of a csv with the summary sentences. 
    
    This function can perform text sumarization with different algorithms one using PageRank 
    and the other using TextRank. The method can be choosen under the methods parameter.
    
    The pagerank method has been implemented locally and all the code is present in this 
    module. This method is much slower, it has been included still because all the code
    is in the module and can be easily understood. To use this method you can save the 
    loaded embeddings after the first time it has been run if you select the correct parameters.
    This can greatly decrease the time it takes to run. Examples are present below. 
    
    The textrank method is from the Gensim package. It uses a variation on textrank where
    the similarity method is using BM25 similarity. Since this implementation is from
    a package the code is not present in this module. 
    ...
    agreement: str
        ...
        be related to the multiple choice questions
    method: str
        Select the summarization method to be used: "pagerank" or "textrank". The pagerank 
        method uses pre-trained embeddings and is a slower method then textrank. 
    output_file: str
        Path to where the summary file will be written out too. The default writes 
        a file to the processed folder within data. If None is choosen then it will
        not write a file.
    embedding_file: str
        Path to the file containing the word embeddings
    embedding: gensim.models.keyedvectors.Word2VecKeyedVectors
        Loaded word vectors, this can help to save time once you have loaded them embeddings
    embedding_return: bool
        If set to true the function will return the loaded embeddings. It will still
        output the summary.
        
    Returns
    -------
    It prints the summary to screen and returns it. If embedding_return is true it also 
    ...
    if depth == "theme":
        if agreement == None:
            ...
            title = "Summary for " + str(name).title() + " - " + str(agreement).title() +" Agreement"
