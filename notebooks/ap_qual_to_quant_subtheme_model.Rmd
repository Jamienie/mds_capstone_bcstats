...
```{r sub_theme summary, echo=FALSE}
# counts by code 
...
# create table
theme_percents <- grouped_data %>% 
  mutate(perc = round(n/sum(n)*100,2)) %>% 
  select(code, diff, perc) %>% 
  spread(diff, perc) %>% 
  rename("Sub-theme" = code,
         "Difference of 0 (%)" = "0",
         "Difference of 1 (%)" = "1",
         "Difference of 2 (%)" = "2") 
...
knitr::kable(
  data_frame(Theme = theme_percents$`Sub-theme`, 
           "Total Number" = as.numeric(counts$n[match(theme_percents$`Sub-theme`, counts$code)]),  
...
ggplot(data=grouped_data) +
  geom_col(aes(x=code, y=n, fill=factor(diff))) +
  labs(fill="Difference", x="sub-theme", y="count") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust = 1)) +
  scale_x_continuous(limits = c(10, 105),
  breaks= scales::pretty_breaks(n=20))
...
- not sure why "Warning message:  Unknown or uninitialised column: 'theme'." keeps appearing

- write some more unit tests/double check filter conditions

- generalize up to the main themes
