...
**add changes that you did to method tonight, ie loss function penalty thign**
...
test_that("Check loss function is taking minimum for each subtheme per person", {
  expect_equal(nrow(test8), 4)
  expect_equal(sum(test8$n), 4)
  })
...
test_that("Check loss function is taking minimum for counts", {
  expect_equal(nrow(test9), 3)
  expect_equal(sum(test9$n), 4)
  })
...
test10 <- table_main_theme_subtheme %>% 
  filter(`Sub-theme` == 12)

test_that("Check proportions are correct", {
  expect_equal(test10$`Total Number`, 103)
  expect_equal(test10$`Strong Agreement (%)`, 50.49)
  expect_equal(test10$`Weak Agreement (%)`, 12.62)
  expect_equal(test10$`No Agreement (%)`, 36.89)
  })
```
...
# column index ayla = 3, aaron = 4, fan = 5 
...
```{r theme and subtheme names, include=FALSE}

appendix_legend <- appendix_subtheme %>%
  rename(theme = Theme,
         code = Code,
         subtheme_description = Description)
                  

write_path = "./references/data-dictionaries/theme_subtheme_names.csv"

write_csv(appendix_legend, write_path)
...
- need to look into why some don't agree
  - try unsupervised techniques like topic modelings, n-grams, etc

- ask about agreement wording of "none" vs "no"

- work on vis for kappa values, look at removing some of the NA issues
  - maybe remove some of Fan's results so they are more conservative
  - group to main themese or something 
  - talk about how there are over 60's groups so no adjustment was used because chance agreement here is much lower then when there are binary classes
...
```{r, include=FALSE}
# looking this was helping manually assement of comments and differences
...
```{r create vis to show sub-theme question match, echo=FALSE}
# grid for plott
a <- rep(1:11, times=6)[1:63]
b <- rep(1:11, each=11)[1:63]
...
total_subtheme <- data_qual %>% 
  ...
  mutate(question = if_else(is.na(question), "no", "has"))
  
total_subtheme %>% 
  group_by(question) %>% 
  summarise(n = n())

# can add conditions to the labels
# so like only ones that don't have mc questions related? or over a threshold?
ggplot(data=total_subtheme) +
  geom_point(aes(x=x_pos, y=y_pos, size=total_comments, color=question)) +
  geom_text(aes(x=x_pos, y=y_pos, label=subtheme),hjust=1.4, vjust=1.3, size=3.2) +
  theme_bw() +
  labs(x="", y="", title="Matching of Subthemes to Multiple Choice Questions") +
  theme(axis.text.x = element_blank(),
  axis.text.y = element_blank(),
  axis.ticks = element_blank()) +
  guides(size=guide_legend(override.aes=list(colour="grey"))) +
  scale_colour_manual(values = c("has"="thistle2", "no"="darkorchid2"))
```
